...
document.getElementById("startBtn").addEventListener("click", async ()=>{
  ...
  scenario = await fetch("./scenario.json").then(r=>r.json());
...
async function finish(){
  ...
  const evidenceRatio = evidence / Math.max(1,totalEvidence);
  let score = Math.round(evidenceRatio * 55 + containment * 0.35 + Math.max(0, 10 - mistakes * 2 - missed));
  score = Math.max(0, Math.min(100, score));
  let rank = score >= 90 ? "Incident Commander" : score >= 78 ? "Threat Investigator" : score >= 64 ? "Security Analyst" : "Junior Responder";
  let outcome, exposure;
  if(containment >= 75 && threat <= 45){ outcome="Contained"; exposure="Prevented"; }
  else if(containment >= 50){ outcome="Partially Contained"; exposure="Limited"; }
  else { outcome="Escalated"; exposure="Likely"; }
  const report = {
    rank, score, outcome, exposure,
    evidenceSummary:`${evidence}/${totalEvidence}`, mistakes, missed, threat, containment,
